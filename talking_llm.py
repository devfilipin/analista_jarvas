...
load_dotenv(find_dotenv())  # Carrega variáveis de ambiente de um arquivo .env

client = openai.Client()  # Inicializa o cliente da OpenAI
...
class TalkingLLM():
    def __init__(self, model="gpt-3.5-turbo-0613", whisper_size="small"):
        ...
        self.whisper = whisper.load_model(whisper_size)  # Carrega o modelo Whisper para transcrição de áudio
        self.llm = ChatOpenAI(model=model)  # Inicializa o modelo de linguagem ChatOpenAI
        self.llm_queue = Queue()  # Inicializa a fila para armazenamento de respostas para TTS
        self.df = None  # Adiciona a propriedade para armazenar o dataframe
        self.create_agent()  # Cria o agente para interagir com o dataframe

        threading.Thread(target=self.convert_and_play, daemon=True).start()  # Inicia a thread para conversão de texto em fala e reprodução do áudio

    def set_display_callback(self, callback):
        self.display_callback = callback  # Define o callback para atualizar a GUI com as respostas
    ...
    def create_agent(self, df_path="df_rent.csv"):
        ...
        try:
            self.df = pd.read_csv(df_path)  # Carrega o dataframe
    ...
    def load_new_dataframe(self, df_path):
        self.create_agent(df_path)  # Carrega um novo dataframe e atualiza o agente

    def save_and_transcribe(self):
        print("Saving the recording...")
        if "test.wav" in os.listdir():
            os.remove("test.wav")  # Remove qualquer arquivo de teste anterior
        wav_file = wave.open("test.wav", 'wb')  # Abre um novo arquivo WAV para escrita
        ...
        wav_file.writeframes(np.array(self.audio_data, dtype=self.dtype))  # Escreve os dados de áudio no arquivo
        wav_file.close()  # Fecha o arquivo WAV

        try:
            result = self.whisper.transcribe("test.wav", fp16=False)  # Transcreve o áudio usando o modelo Whisper
            user_text = result["text"]  # Obtém o texto transcrito
            print("Usuário:", user_text)

            response = self.agent.invoke(user_text)  # Obtém a resposta do agente para o texto transcrito
            ...
            if self.display_callback:
                self.display_callback(user_text, jarvas_response)  # Chama o callback para atualizar a GUI, se definido
    ...
    def start_recording_thread(self):
        threading.Thread(target=self.run_recording, daemon=True).start()  # Inicia a thread de gravação
